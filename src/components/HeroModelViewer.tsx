...
const ModelContentContext = createContext<THREE.Object3D | null>(null);
...
const DesktopControls: FC<{
  ...
  useFrame(() => {
    if (ref.current?.target) ref.current.target.copy(pivot);
  });
  return (
    <OrbitControls
...
function GLTFContent({
  ...
  url: string;
  innerProps: ModelInnerWrapperProps;
}) {
  const { scene } = useGLTF(url);
  const cloned = useMemo(() => scene.clone(), [scene]);
  return (
    <ModelContentContext.Provider value={cloned}>
      <ModelInnerWrapper {...innerProps} />
...
const ModelInnerWrapper: FC<ModelInnerWrapperProps> = (props) => {
  const content = useContext(ModelContentContext);
  ...
  return (
    <ModelInner
      ...
      yOff={props.yOff ?? 0}
      pivot={props.pivot!}
      initYaw={props.initYaw ?? 0}
      initPitch={props.initPitch ?? 0}
      ...
      autoFrame={props.autoFrame ?? false}
      fadeIn={props.fadeIn ?? false}
        ...
        g.traverse((o: THREE.Object3D) => {
          if (isMeshObject(o)) {
            const materials = Array.isArray(o.material)
              ...
              : [o.material];
            materials.forEach((material: THREE.Material) => {
              (material as THREE.MeshBasicMaterial).opacity = v;
            });
          }
        });
        invalidate();
...
const HeroModelViewer: FC<HeroModelViewerProps> = ({
  url,
  width = 400,
  height = 400,
  modelXOffset = 0,
    ...
    >
      {showScreenshotButton && (
        <button
          type="button"
          onClick={capture}
          className="absolute right-4 top-4 z-10 cursor-pointer rounded-[10px] border border-white/80 bg-black/40 px-4 py-2 text-sm text-white backdrop-blur-sm hover:bg-black/60"
        >
          Take Screenshot
        </button>
      )}
      ...
      >
        {environmentPreset !== "none" && (
          <Environment
            preset={environmentPreset as "forest"}
            background={false}
          />
        )}
        ...
        <Suspense fallback={<Loader placeholderSrc={placeholderSrc} />}>
          {ext === "glb" || ext === "gltf" ? (
            <GLTFContent url={url} innerProps={innerProps} />
          ) : ext === "fbx" ? (
            <FBXContent url={url} innerProps={innerProps} />
          ) : ext === "obj" ? (
            <OBJContent url={url} innerProps={innerProps} />
          ) : null}
        </Suspense>
