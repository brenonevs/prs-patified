...
export function GridScan({
  ...
  const [faceApiModule, setFaceApiModule] = useState<FaceApiModule | null>(null);
  ...
  useEffect(() => {
    ...
    import("face-api.js").then((mod) => {
      if (!canceled) setFaceApiModule(mod);
    });
    return () => {
      canceled = true;
    };
  ...
  useEffect(() => {
    if (!enableWebcam || !faceApiModule) return;
    let canceled = false;
    Promise.all([
      faceApiModule.nets.tinyFaceDetector.loadFromUri(modelsPath),
      faceApiModule.nets.faceLandmark68TinyNet.loadFromUri(modelsPath),
    ])
      .then(() => {
  ...
  }, [enableWebcam, faceApiModule, modelsPath]);
    ...
    const onClick = async () => {
      ...
      if (enableGyro && typeof window !== "undefined") {
        try {
          ...
          if (DevOrient?.requestPermission) await DevOrient.requestPermission();
        } catch {
        }
      }
    };
  ...
  useEffect(() => {
    ...
    if (enablePost) {
      ...
      (bloom.blendMode as { opacity: { value: number } }).opacity.value = Math.max(
        0,
        bloomIntensity
      );
    ...
    const onResize = () => {
      renderer.setSize(container.clientWidth, container.clientHeight);
      (
        material.uniforms.iResolution as { value: THREE.Vector3 }
      ).value.set(
        container.clientWidth,
        container.clientHeight,
        renderer.getPixelRatio()
    ...
    const tick = () => {
      ...
      (material.uniforms.uSkew as { value: THREE.Vector2 }).value.set(
        skew.x,
        skew.y
      );
      (material.uniforms.uTilt as { value: number }).value =
        tiltCurrent.current * tiltScale;
      (material.uniforms.uYaw as { value: number }).value =
        THREE.MathUtils.clamp(yawCurrent.current * yawScale, -0.6, 0.6);

      (material.uniforms.iTime as { value: number }).value = now / 1000;
  ...
  useEffect(() => {
    ...
    if (bloom) {
      (bloom.blendMode as { opacity: { value: number } }).opacity.value =
        Math.max(0, bloomIntensity);
    }
    const chroma = chromaRef.current;
    if (chroma) {
      chroma.offset.set(chromaticAberration, chromaticAberration);
    }
  }, [
    lineThickness,
    linesColor,
  ...
  useEffect(() => {
    ...
    const start = async () => {
      if (!enableWebcam || !modelsReady || !faceApiModule) return;
      ...
      const opts = new faceApiModule.TinyFaceDetectorOptions({
        inputSize: 320,
        scoreThreshold: 0.5,
      });
      ...
      const detect = async (ts: number) => {
          ...
          try {
            ...
            if (res?.detection) {
              ...
              lookTarget.current.copy(
                look.multiplyScalar(depthScale)
              );
              ...
              const eyeDist =
                Math.hypot(rc.x - lc.x, rc.y - lc.y) + 1e-6;
        ...
        if (vid && "requestVideoFrameCallback" in HTMLVideoElement.prototype) {
          (vid as HTMLVideoElement & { requestVideoFrameCallback: (cb: () => void) => number }).requestVideoFrameCallback(
  ...
  }, [enableWebcam, modelsReady, depthResponse, faceApiModule]);
